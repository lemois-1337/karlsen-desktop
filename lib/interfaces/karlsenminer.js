...
module.exports = class Simulator extends Daemon {
	...
	async start() {
		...
		let defaults = {
			...
			// dbuser: 	'karlsen', // Database user
			// dbpass: 	'karlsen', // Database password
...
//		let {config:daemons} = await this.get("get-modules-config");
		...
		if(!args.miningaddr) {
			args.miningaddr = 'karlsentest:qpjmmd8fc20c6mf5zxr6qax0369g0aufyg6aqcel9u';
			//args.miningaddr = 'karlsen:qqg767gkznmsuah5xeer9f72lwqu642nqvlmujtpmk';
			//args.miningaddr = 'karlsen:qr35ennsep3hxfe7lnz5ee7j5jgmkjswsslwxj42ta';
		}		
		...
		//console.log("++++++++++++++++ KARLSENMINER VSARGS")

		await this.VSARGS([this.getDeployableBinary('karlsenminer'), '--version']);

		args = Object.entries(args);

		const flags = utils.args();
		//console.log("++++++++++++++++ KARLSENMINER FLAGS",flags)

		this.options = {
			args : () => {
				// this.log(`--addresslist=${configFile}`,`--cert=${rpcCertFile}`);
				let _args_ = [
					this.getBinary('karlsenminer'),
				...
				console.log("---KARLSENMINER ARGS",a);
			...
			stdout : (data) => { 
				...
				// // console.log("karlsenminer match:", match);
		...
		//console.log("KARLSENMINER STATUS:",this.manager.enableMining,flags.mine);
		if(this.manager.enableMining || flags.mine) {
			dpc(0, () => {
				//console.log("----------------- KARLSENMINER STARTING")
				this.run()
			})
		} else {
			//console.log("----------------- KARLSENMINER NOT RUNNING")
			this.state = 'disabled';
			dpc(0, () => {
				this.writeToSink('\r\n\r\nKarlsenminer is disabled. Please use Enable Mining option to start it.\r\n\r\n');
			})
		}
