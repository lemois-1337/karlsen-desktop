...
const {Client:KarlsenGRPC} = require('@karlsen/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Karlsend extends Daemon {
	...
	start() {
		this.log("karlsend::start() invoked...");
		return new Promise(async (resolve,reject) => {

			if(this.task.conf['reset-peers'])
				this.cleanupResidualData();

			this.log("karlsend::start() running");
			...
			if (!("uacomment" in args)) {
				const uacomment = `karlsen-desktop_${pkg['version']}`
				this.writeToSink(`KARLSEND: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`KARLSEND: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`KARLSEND: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`KARLSEND: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`KARLSEND: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'karlsend.conf');
			...
			await this.VSARGS([this.getDeployableBinary('karlsend'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('karlsend')
					];
			...
			resolve();
			// mutes all karlsend processes by default
			this.mute = true;
		})
	}
			...
			${
				this.data.map((o) => {
					...
					if(time) {
						...
						css += 'min-width:120px;'
					} else if(/^(network hashrate|difficulty|mempool|median latency)/i.test(o.name)) {
						css += 'min-width:160px;'
					}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				if(this.syncData && blockCount > 1) {
					...
					// Human readable network hashrate.
					hashrateSample = bdi.difficulty * 2;
					if (hashrateSample >= 1000000000000000) {
						hashrateValue = (hashrateSample / 1000000000000000).toFixed(2)+' PH/s';
					} else if (hashrateSample >= 1000000000000) {
						hashrateValue = (hashrateSample / 1000000000000).toFixed(2)+' TH/s';
					} else if (hashrateSample >= 1000000000) {
						hashrateValue = (hashrateSample / 1000000000).toFixed(2)+' GH/s';
					} else if (hashrateSample >= 1000000) {
						hashrateValue = (hashrateSample / 1000000).toFixed(2)+' MH/s';
					} else if (hashrateSample >= 1000) {
						hashrateValue = (hashrateSample / 1000).toFixed(2)+' KH/s';
					} else {
						hashrateValue = hashrateSample.toFixed(2)+' H/s';
					}
				}
				else {
					syncSample = 0;
				...
				const sources =  {
					...
					"Network Hashrate" : { sample : hashrateSample, value : hashrateValue, graph : true },
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('KARLSEND monitor:'.bold,ex);
	            return reject(ex);
	        }
