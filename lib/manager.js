...
class KarlsenProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['karlsend','mqtt','mysql','pgsql', 'gpuminer', 'karlsenminer','txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['karlsend','mqtt','mysql','pgsql','simulator','txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.karlsen-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, karlsen } = pkg;
			return {
				folder, name, description, main, engines, ...karlsen
			}
		});
	}
...
module.exports = KarlsenProcessManager;
